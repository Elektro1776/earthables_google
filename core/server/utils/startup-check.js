var packages = require('../../../package.json'), path = require('path'),
    crypto = require('crypto'), fs = require('fs'),
    mode = process.env.NODE_ENV === undefined ? 'development'
                                              : process.env.NODE_ENV,
    appRoot = path.resolve(__dirname, '../../../'),
    configFilePath =
        process.env.GHOST_CONFIG || path.join(appRoot, 'config.js'),
    checks, exitCodes = {
...
checks = {
  ...
  nodeVersion: function checkNodeVersion() {
      ...
      if (process.env.GHOST_NODE_VERSION_CHECK !== 'false' &&
          ...
          console.error('\x1B[31mGhost needs Node version ' +
  packages.engines.node +
                        ' you are using version ' + process.versions.node +
  '\033[0m\n');
          console.error('\x1B[32mPlease see
  http://support.ghost.org/supported-node-versions/ for more
  information\033[0m');
...
*/
  nodeEnv : function checkNodeEnvState() {
    // Check if config path resolves, if not check for NODE_ENV in
    // config.example.js prior to copy
    var fd, configFile, config;
    ...
    if (!config) {
      console.error(
          '\x1B[31mERROR: Cannot find the configuration for the current NODE_ENV: ' +
          process.env.NODE_ENV + '\033[0m\n');
      console.error(
          '\x1B[32mEnsure your config.js has a section for the current NODE_ENV value' +
  ...
  packages : function checkPackages() {
    ...
    console.error(
        '\x1B[31mERROR: Ghost is unable to start due to missing dependencies:\033[0m\n  ' +
        errors);
    console.error(
        '\x1B[32m\nPlease run `npm install --production` and try starting Ghost again.');
    console.error(
        '\x1B[32mHelp and documentation can be found at http://support.ghost.org.\033[0m\n');
  ...
  contentPath : function checkContentPaths() {
    ...
    var configFile, config, contentPath,
        contentSubPaths = [ 'apps', 'data', 'images', 'themes' ], fd,
        errorHeader =
            '\x1B[31mERROR: Unable to access Ghost\'s content path:\033[0m',
        errorHelp =
            '\x1B[32mCheck that the content path exists and file system permissions are correct.' +
            '\nHelp and documentation can be found at http://support.ghost.org.\033[0m';

    // Get the content path to test.  If it's defined in config.js use that, if
    // not use the default
    try {
      configFile = require(configFilePath);
      config = configFile[mode];
    ...
    // Use all sync io calls so that we stay in this function until all checks
    // are complete
  ...
  sqlite : function checkSqlite() {
    ...
    var configFile, config, appRoot = path.resolve(__dirname, '../../../'),
                            dbPath, fd;
    ...
    try {
      ...
      dbPath = path.join(appRoot, 'content', 'data',
                         mode === 'production' ? 'ghost.db' : 'ghost-dev.db');
    }
    ...
    try {
      ...
      console.error(
          '\x1B[31mERROR: Unable to open sqlite3 database file for read/write\033[0m');
      console.error('  ' + e.message);
      console.error(
  ...
  mail : function checkMail() {
    ...
    if (!config.mail || !config.mail.transport) {
      console.error(
          '\x1B[31mWARNING: Ghost is attempting to use a direct method to send email. \nIt is recommended that you explicitly configure an email service.\033[0m');
      console.error(
          '\x1B[32mHelp and documentation can be found at http://support.ghost.org/mail.\033[0m\n');
    }
  },

  builtFilesExist : function builtFilesExist() {
    var configFile, config, location,
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      fileNames =
          fileNames.map(function(file) { return file.replace('.', '.min.'); });
    }
    ...
    function checkExist(fileName) {
      try {
        ...
        console.error(
            '\x1B[31mERROR: Javascript files have not been built.\033[0m');
        console.error(
            '\n\x1B[32mPlease read the getting started instructions at:');
        console.error(
            'https://github.com/TryGhost/Ghost#getting-started\033[0m');
        process.exit(exitCodes.BUILT_FILES_DO_NOT_EXIST);
      }
    }

    fileNames.forEach(function(fileName) { checkExist(location + fileName); });
  }
};
